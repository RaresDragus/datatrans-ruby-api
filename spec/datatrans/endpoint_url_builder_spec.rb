...
RSpec.describe Datatrans::EndpointUrlBuilder do
  describe '.build' do
    it 'returns the correct url for Aliases convert' do
      expect(
        described_class.build(action: :convert, env: :live, service: 'Aliases', version: 1)
      ).to eq('https://api.datatrans.com/v1/aliases')
    end

    it 'returns the correct url for Aliases delete' do
      expect(
        described_class.build(action: :delete, env: :test, id: 'ABCdef', service: 'Aliases', version: 1)
    ...
    it 'returns the correct url for Reconciliations sales_bulk' do
      expect(
        described_class.build(action: :sales_bulk, env: :live, service: 'Reconciliations', version: 1)
    ...
    it 'returns the correct url for Transactions settle' do
      expect(
        described_class.build(action: :settle, env: :live, id: 'ABCdef', service: 'Transactions', version: 1)
      ).to eq('https://api.datatrans.com/v1/transactions/ABCdef/settle')
    end

    it 'returns the correct url for Transactions cancel' do
      expect(
        described_class.build(action: :cancel, env: :test, id: 'ABCdef', service: 'Transactions', version: 1)
    ...
    it 'returns the correct url for Transactions status' do
      expect(
        described_class.build(action: :status, env: :test, id: 'ABCdef', service: 'Transactions', version: 1)
      ).to eq('https://api.sandbox.datatrans.com/v1/transactions/ABCdef')
    end

    it 'returns the correct url for Transactions update_amount' do
      expect(
        described_class.build(action: :update_amount, env: :live, id: 'ABCdef', service: 'Transactions', version: 1)
    ...
    it 'raises error if id is required but is blank' do
      expect do
        described_class.build(
          action: %i[authorize_with_transaction cancel credit_with_transaction delete status settle
                     update_amount].sample,
          env: :live,
